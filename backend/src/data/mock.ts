...
const RAW: [string, string, [string, string, boolean][]][] = [
  ...
  ['Worlds','Distortions',[
    ...
    ['Throneworld Case File Collector','Monde du Trône',false],
  ]],
  ['Stories','Tower',[
...
// Progression initiale mockée : Bibullus a les triomphes marqués done:true
const DONE_BY_BIBULLUS = new Set(
  RAW.flatMap(([, , items], groupIdx) =>
    items
      .map(([, , done], itemIdx) => ({ done, id: `t${RAW.slice(0, groupIdx).reduce((a, [,, it]) => a + it.length, 0) + itemIdx}` }))
      .filter(({ done }) => done)
      .map(({ id }) => id)
  )
);
